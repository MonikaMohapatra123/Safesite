import React, { useState, useEffect } from "react";
import axios from "axios";
import './AdminBlog.css';

const BACKEND_URL = process.env.REACT_APP_BACKEND_URL || "http://localhost:5000";

const AdminBlog = () => {
  const [blogs, setBlogs] = useState([]);
  const [editingId, setEditingId] = useState(null);
  const [blogData, setBlogData] = useState({
    page: "ResourceBlog",
    featured: {
      image: "",
      tag: "",
      readTime: "",
      title: "",
      description: "",
      author: "",
      date: ""
    },
    posts: [],
    blogList: []
  });

  // Fetch all blogs
  const fetchBlogs = async () => {
    try {
      const res = await axios.get(`${BACKEND_URL}/api/blogs`);
      setBlogs(res.data);
    } catch (err) {
      console.error(err);
      alert("Error fetching blogs");
    }
  };

  useEffect(() => {
    fetchBlogs();
  }, []);

  // Load blog data when editing
  useEffect(() => {
    if (editingId) {
      const blog = blogs.find(b => b._id === editingId);
      if (blog) setBlogData(blog);
    } else {
      resetBlogData();
    }
  }, [editingId, blogs]);

  const resetBlogData = () => {
    setBlogData({
      page: "ResourceBlog",
      featured: {
        image: "",
        tag: "",
        readTime: "",
        title: "",
        description: "",
        author: "",
        date: ""
      },
      posts: [],
      blogList: []
    });
  };

  // Handle featured field change
  const handleFeaturedChange = (e) => {
    const { name, value } = e.target;
    setBlogData({ ...blogData, featured: { ...blogData.featured, [name]: value } });
  };

  // Handle submit
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingId) {
        await axios.put(`${BACKEND_URL}/api/blogs/${editingId}`, blogData);
      } else {
        await axios.post(`${BACKEND_URL}/api/blogs`, blogData);
      }
      fetchBlogs();
      setEditingId(null);
      resetBlogData();
      alert("Blog saved successfully!");
    } catch (err) {
      console.error(err);
      alert("Error saving blog");
    }
  };

  // Delete blog
  const deleteBlog = async (id) => {
    if (window.confirm("Are you sure you want to delete this blog?")) {
      try {
        await axios.delete(`${BACKEND_URL}/api/blogs/${id}`);
        fetchBlogs();
      } catch (err) {
        console.error(err);
        alert("Error deleting blog");
      }
    }
  };

  return (
    <div className="admin-blog-container">
      <h2>{editingId ? "Edit Blog" : "Add New Blog"}</h2>

      <form onSubmit={handleSubmit} className="blog-form card">
        <h3>Featured Section</h3>
        {["image","tag","readTime","title","description","author","date"].map(field => (
          <div className="form-group" key={field}>
            <label>{field.charAt(0).toUpperCase() + field.slice(1)}</label>
            <input
              name={field}
              value={blogData.featured[field]}
              onChange={handleFeaturedChange}
              placeholder={`Enter ${field}`}
            />
          </div>
        ))}

        <button type="submit" className="submit-btn">
          {editingId ? "Update Blog" : "Add Blog"}
        </button>
      </form>

      <hr />

      <h3>Existing Blogs</h3>
      <ul className="blogs-list">
        {blogs.map((b) => (
          <li key={b._id} className="blog-item">
            <strong>{b.featured.title}</strong> ({b.page})
            <div className="blog-actions">
              <button className="edit-btn" onClick={() => setEditingId(b._id)}>Edit</button>
              <button className="delete-btn" onClick={() => deleteBlog(b._id)}>Delete</button>
            </div>
          </li>
        ))}
      </ul>
    </div>
  );
};

export default AdminBlog;
